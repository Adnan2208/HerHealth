// HerHealth custom illustration set. Flat-vector, warm healthcare palette, culturally
...
const SKIN = '#c98a5f'
const SKIN_D = '#a86c46'
const SARI = '#ABC798'        // Primary healthcare / sage accent
const SARI_L = '#FFC4EB'      // Feminine soft pink accent
const GOLD = '#D5CEAB'        // Soft beige / subtle gold
const CREAM = '#FFE4FA'       // Very light pink foundation
const APRICOT = '#F1DEDC'     // Warm blush beige
const SAGE = '#ABC798'        // Muted sage green
const LEAF = '#7A9C69'        // Leaf/natural sage
const INK = '#2C362B'         // Deep readable neutral
...
export function HeroIllustration() {
  return (
    <Frame>
      <ellipse cx="120" cy="170" rx="92" ry="24" fill="#F1DEDC" />
      <ellipse cx="120" cy="150" rx="86" ry="58" fill="#FFE4FA" opacity="0.6" />
      {/* woman */}
      ...
      <path d="M96 92c8-6 16-9 24-9s16 3 24 9l2 14H94z" fill={SARI_L} opacity="0.6" />
      ...
      <rect x="154" y="102" width="36" height="54" rx="7" fill="#FFFFFF" />
      <ellipse cx="172" cy="124" rx="13" ry="9" fill="#fff" stroke={SKIN_D} strokeWidth="2" />
      <ellipse cx="172" cy="124" rx="7" ry="5" fill={SARI} />
...
export function EyeArt({ variant = 'guide' }) {
  if (variant === 'good') {
    return (
      <Frame viewBox="0 0 200 120">
        ...
        <ellipse cx="100" cy="60" rx="34" ry="24" fill="#ABC798" />
        <ellipse cx="100" cy="60" rx="16" ry="13" fill="#2C362B" />
  ...
  return (
    <Frame viewBox="0 0 220 150">
      ...
      <ellipse cx="110" cy="72" rx="36" ry="26" fill="#ABC798" />
      <ellipse cx="110" cy="72" rx="17" ry="14" fill="#2C362B" />
      ...
      <text x="110" y="126" textAnchor="middle" fontSize="11" fontWeight="800" fill="#1E2D1B" fontFamily="inherit">keep red part here</text>
    </Frame>
  )
}
...
export function ScanProcessingArt() {
  return (
    <Frame viewBox="0 0 200 150" title={undefined}>
      <ellipse cx="100" cy="110" rx="70" ry="16" fill="#F1DEDC" />
      <ellipse cx="100" cy="70" rx="62" ry="42" fill="#fff" stroke={SARI} strokeWidth="3" />
      <ellipse cx="100" cy="70" rx="30" ry="22" fill="#ABC798" />
      <ellipse cx="100" cy="70" rx="14" ry="12" fill="#2C362B" />
      <rect className="scan-line-rect" x="44" y="66" width="112" height="5" rx="2.5" fill="#ABC798" />
...
const STORY_SCENES = {
  ...
  t5: (<><ellipse cx="120" cy="120" rx="58" ry="26" fill="#fff" stroke={SAGE} strokeWidth="3" /><ellipse cx="100" cy="106" rx="16" ry="10" fill={LEAF} transform="rotate(-20 100 106)" /><ellipse cx="126" cy="104" rx="14" ry="9" fill={LEAF} transform="rotate(14 126 104)" /><circle cx="146" cy="118" r="12" fill={SARI_L} /><circle cx="92" cy="120" r="9" fill={GOLD} /><path d="M120 60c8 0 14 2 18 6" stroke={GOLD} strokeWidth="3" fill="none" strokeLinecap="round" /></>),
  ...
  t7: (<><path d="M120 50s26 30 26 50a26 26 0 0 1-52 0c0-20 26-50 26-50z" fill="#fff" stroke={SARI} strokeWidth="3" /><path d="M120 72s14 16 14 28a14 14 0 0 1-28 0c0-12 14-28 14-28z" fill={SARI_L} /><rect x="70" y="136" width="100" height="10" rx="5" fill={GOLD} /><rect x="70" y="136" width="38" height="10" rx="5" fill="#b3261e" /></>),
...
export function StoryArt({ id }) {
  return (
    <Frame viewBox="0 0 240 180">
      <ellipse cx="120" cy="152" rx="88" ry="20" fill="#F1DEDC" />
...
export function FoodArt({ id }) {
  return (
    <svg viewBox="0 0 120 110" aria-hidden="true" focusable="false">
      <circle cx="60" cy="58" r="52" fill="#FFFFFF" />
      <circle cx="60" cy="58" r="52" fill="none" stroke="#E1DABD" strokeWidth="2" />
...
export function EmptyArt({ kind = 'results' }) {
  if (kind === 'history') {
    return (
      <Frame viewBox="0 0 220 150">
        <rect x="40" y="30" width="140" height="90" rx="16" fill="#fff" stroke={GOLD} strokeWidth="3" />
        <path d="M70 50h80M70 66h80M70 82h50" stroke={APRICOT} strokeWidth="8" strokeLinecap="round" />
        <circle cx="150" cy="96" r="18" fill={CREAM} stroke={SARI_L} strokeWidth="3" />
  ...
  if (kind === 'alerts') {
    return (
      <Frame viewBox="0 0 220 150">
        <rect x="70" y="40" width="80" height="62" rx="12" fill="#fff" stroke={GOLD} strokeWidth="3" />
        ...
        <ellipse cx="110" cy="122" rx="52" ry="10" fill="#F1DEDC" />
      </Frame>
    )
  }
  return (
    <Frame viewBox="0 0 220 150">
      <ellipse cx="110" cy="122" rx="56" ry="12" fill="#F1DEDC" />
...
export function TrustArt({ mini = false }) {
  return (
    <svg viewBox="0 0 96 96" className={mini ? 'trust-mini' : 'illus'} aria-hidden="true" focusable="false" style={mini ? undefined : { width: 96, height: 96 }}>
      <circle cx="48" cy="48" r="44" fill="#EAF4E3" />
